// koe boot smoke (koe-ef8 Step A). The app is wrapped in OnboardingGate
// (src/features/settings/OnboardingGate.tsx): on a FRESH profile it renders
// "読み込み中…" then the wizard heading "koe へようこそ" once settings load.
...
describe("koe boot smoke", () => {
  it("launches on Windows and shows the onboarding welcome screen", async () => {
    // h1.koe-onboarding-heading is rendered by the onboarding wizard branch.
    const heading = await $("h1.koe-onboarding-heading");
    await heading.waitForExist({ timeout: 60000 });
    await expect(heading).toHaveText("koe へようこそ");
  });
});
