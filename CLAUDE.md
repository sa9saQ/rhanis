...
## Branches / Milestones
- `main`: M1 backend 完成（PR #1–#23 merged）。cost_tracker / secret_store（stronghold BYOK + multi-provider 対応 koe-31u）/ activity 可視化 / recorder（SQLite）/ settings + 予算オンボーディング + 声 provider / 手足 tool キー UI / approval_gate（SAFE・CAUTION・DANGER の 3 段 + 同時 pending DANGER 承認 cap = modal-flood guard koe-rxh #23）/ tool_dispatcher（in-flight 上限 DoS guard koe-wj2）/ session_manager（WebSocket + RealtimeAuth + コスト gate）/ audio_bridge（cpal マイク + rodio 再生 + clippy never_loop/absurd 整理 koe-a4h #22）/ M1 tools 4 本 が実装・マージ済
...
- **製品方向（2026-06）**: multi-provider キー設定基盤 `koe-31u` ✅ merged（#20。声 = OpenAI / Google 選択 + 手足 tool キーの暗号保管 + 設定 UI）→ **着手中**: 接続層 trait 化 `koe-zv3`（P1, in_progress。Plan 承認済 `~/.claude/plans/refactored-exploring-acorn.md`, worktree `~/projects/koe-zv3` 準備済, 実装着手前のクリーン中断点。**PR1** = `RealtimeProvider` trait 抽象化 + OpenAI 切り出しの挙動不変リファクタ / **PR2** = `GeminiLive` impl + 実選択 + audio 16kHz）+ 手足 tool `koe-eal`（x_search 等を tool_dispatcher に追加。`tool_providers` フラグと各プロバイダキーを consume）。許可ポリシー UI は `koe-351`（settings 層を koe-31u と共有するので衝突注意）
- **follow-up（review 派生）**: `koe-rxh` ✅ merged（#23, 承認待ち cap = modal-flood + approval-map guard）/ `koe-a4h` ✅ merged（#22, clippy never_loop/absurd）→ 新 follow-up `koe-e2b`（P2, koe-rxh の R-C で Codex 捕捉。approval cap は `register()` で効くが dispatch slot 消費は task `spawn` 時点なので、64 DANGER 連射の spawn-burst で一時的に starvation が残る。根本解決 = spawn 前の risk-aware admission（DispatcherSeam に `try_admit`）。**同じ run_read_loop を触る koe-zv3 と整合的に設計**、approval cap は backstop として残す）
