...
- **M1 残（2026-06-04 徹底レビューで再定義）**: backend コアは完成だが、**E2E（`koe-ef8`）は実装ギャップで現在 blocked**（旧「依存全✓着手可」は楽観的だった）。先に潰す M1 完成タスク: `koe-emd`（会話ログ配線=`log_conversation_event` 本番呼び出し 0 件）/ `koe-9xi`（コスト残高ライブ表示=cost_tracker 実装済だが front 未読、E2E 完了条件④の前提）/ `koe-iyr`（オンボーディング無スタイル修正）。並行: `koe-471`（ApprovalModal a11y）/ `koe-5sc`（IPC parity test）/ `koe-0my`（CI 起動・現状 `.github` が壊れた device で workflow 0 件）/ `koe-eco`（lockfile commit, P1 昇格）/ `koe-8t2`（マイク権限 UX）/ `koe-30t`（初回チュートリアル）。hardening は Windows 実機: `koe-pr3` audio race / `koe-8kw` read_file handle walk / `koe-2br`（usage payload 確定→fail-closed 化, P1 昇格・`koe-ef8` 依存）。WSL ではマイク（cpal）不可なので E2E は Windows 必須
