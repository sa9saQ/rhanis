...
> ⚠️ **2026-06-10 更新（デザイン方針 pivot）**: 本書 §1-§8 のデザイン提言は「没入 orb 一個」前提だが、ユーザー判断で **「見える glass-box コンソール + 音声主役」へ pivot 済**（orb は縮小した音声状態インジケータに格下げ）。**現行の正 = `docs/design/2026-06-10-glassbox-console-design-brief.md` / 本書 §9 / bd `koe-ios`**。§4 の orb craft（モーション/配色/二段平滑化等）は縮小インジケータに流用するため有効、それ以外の「orb を画面主役にする」記述は superseded。
