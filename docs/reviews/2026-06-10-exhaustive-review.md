...
> **追記（同日 user 決定、レポート確定後に上書き）**: ①**決済 = Stripe で確定**（koe-3x6 記録済み既定どおり。§4.5 の Polar 推奨は撤回。Stripe JP は固定手数料なしのため §4.3 の「¥500 パック廃止」も撤回 — card testing 対策としての最低額引き上げは任意の選択肢に格下げ。残る課題は EU VAT のみ = 当面 EU 販売制限 or **Stripe Managed Payments**〔Stripe 自身の MoR、Lemon Squeezy 後継、2026-02 public preview。日本セラー可否は M4 設計時に要確認〕）。手数料 3.6-5.6% 前提で定常下限は M≈1.15-1.21 に微緩和、**推奨 M=1.8-2.0 は不変**。②**無料お試しは「任意」の既決定が正**（session-decisions §5「時間制トライアルは任意」/ koe-3x6 note「無くてもよい、user」。§9 提案 6 の plan 矛盾は「任意」側で解消）— §4.4 の数値は実施する場合の参考値であり、**M4 初期はトライアル無し開始を推奨**（SMS 認証 + Sybil 防御の実装が後回しになる）。
...
- ~~**¥500 チャージパック廃止**~~ **【同日 user 決定で撤回 — §4 冒頭参照】**: 以下は MoR 前提の議論。**決済 = Stripe 確定（固定手数料なし）のため ¥500 チャージは廃止不要**。最低チャージ引き上げ（¥1,500〜）は card testing（Visa VAMP の excessive 閾値が 2026-04 から 1.5% へ強化）対策としての任意の選択肢に格下げ。
...
- ~~**決済は Polar 一本を推奨**~~ **【同日 user 決定で撤回 → 決済 = Stripe 確定。§4 冒頭参照】**: 以下は撤回された当初推奨の記録。「Polar（MoR 5%+$0.50、日本セラー対応、dispute $15）が個人のグローバル販売に向く、Stripe 直販は EU VAT が 1 件目から課されるため非現実的」と書いたが、**user は Stripe 確定（記録済み既定どおり）**。EU VAT 問題は当面 EU 販売制限 or Stripe Managed Payments（Stripe 自身の MoR、2026-02 preview、日本セラー可否は M4 で確認）で対処。Paddle は $10 未満要相談、Lemon Squeezy は Stripe 買収後の移行期で新規非推奨、は参考情報として有効。
...
| 7 | **koe-krv P/L ゲート確定（素材は本レポート §4 で揃った）** | M=1.8-2.0 / 安価既定ロック / 180 日失効 / 決済=Stripe確定 / トライアル任意（初期無し開始）をプラン額として固定 |
...
- **同日セッション close での追加反映（user 決定 6 件 + 整合性）**: ①決済 = Stripe 確定 / 無料お試し = 任意（koe-3x6・koe-krv note + §4 追記ブロック + CLAUDE.md 収益モデル節）②M1 着手順（5fs 1 行 → ef8 → 解禁後 ds6、koe-ef8/koe-5fs note）③table-stakes + 校正体験採用（koe-bx7/944/1l4/nak/84w note）④koe-1mf にマージン赤字設計の訂正 note ⑤koe-20f に戦略 stress-test の適用 note ⑥related リンク 4 件（3x6↔6ul/y1j/7yy、yb4↔1mf）⑦**plan（virtual-riding-hearth.md）の stale 8 箇所修正**（CSP / M4 旧 2 モード対等 / マージン +15-25% / トライアル必須 / Hermes 命名 ×2 / BYOK 買い切り未決注記 / P/L ゲート結線）⑧session-decisions §5・competitive-landscape に精緻化注記 ⑨bd memory `koe-2026-06-10-exhaustive-review` を決定込み最終版に更新
...
5. ~~soft link 追加: yb4↔1mf / 6ul・y1j・7yy→3x6~~ → **同日実施済み**（related リンク 4 件）
6. ~~**plan（virtual-riding-hearth.md）の stale 記述修正**~~ → **同日実施済み**（8 箇所: CSP / M4 旧 2 モード対等 / マージン +15-25% → ×1.8-2.0 / トライアル必須 → 任意 / Hermes 命名 / BYOK 買い切りに koe-bup 未決注記 / P/L ゲート結線。「標準/高品質」ラベルは plan L40/L217 で既に撤回注記済みを確認）
